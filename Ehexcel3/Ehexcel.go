...
func ehexcel(records [][][]string, opt *option) {
	...
	templateExcl, _ := ecxl.Open(opt.templateXlsx)
	defer templateExcl.Close()

	for i, record := range records {
		sheetName := sheets[(opt.sheetNumbers[i] - 1)].Name
		// fmt.Println(opt.sheetNumbers)

		sheet, _ := templateExcl.OpenSheet(sheetName)
		for y, line := range record {
			offsetY := y + opt.startYs[i] + 1
			for x, v := range line {
				offsetX := x + opt.startXs[i] + 1
				sheet.GetRow(offsetY).GetCell(offsetX).SetString(v)
			}
		}
	}

	err = templateExcl.Save(opt.outputXlsx)
	if err != nil {
		util.Fatal(err, util.ExitCodeFileOpenErr)
	}
}
